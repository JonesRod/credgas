...
<!DOCTYPE html> 
    ...
    <body> 
        <form action="atualizar_produto.php" method="POST" enctype="multipart/form-data"> 
            <h1>Editar Produto</h1> <input type="hidden" id="id_parceiro" name="id_parceiro" value="<?php echo $produto['id_parceiro']; ?>"> <input type="hidden" name="id_produto" value="<?php echo htmlspecialchars($produto['id_produto']); ?>"> 
            <input type="hidden" id="imagens_salvas" name="imagens_salvas" value="<?php echo htmlspecialchars($produto['imagens']); ?>"> 
            <input type="hidden" id="imagens_removidas" name="imagens_removidas" value=""> <!-- Nome do Produto --> 
            <div class="form-group"> <label for="nome_produto">Nome do Produto:</label> 
                <input type="text" id="nome_produto" name="nome_produto" value="<?php echo htmlspecialchars($produto['nome_produto']); ?>" obrigatório> 
            </div> 
            <!-- Descrição do Produto --> 
             <div class="form-group"> 
                <label for="descricao_produto">Descrição do Produto:</label> <textarea id="descricao_produto" name="descricao_produto" rows="4" obrigatório><?php echo htmlspecialchars($produto['descricao_produto']); ?></textarea> </div> <!-- Valor do Produto -- > <div class="form-group"> <!-- Converter o valor do produto para float e formato --> <?php $valor_produto = str_replace(',', '.', $produto['valor_produto']); valor="<?php eco number_format($valor_produto, 2, ',', '.'); ?>" obrigatório oninput="formatarValor(this)"> </div> <!-- Valor do Produto + Taxa --> <div class="form-group"> <label for="valor_produto_taxa">Valor do Produto + Taxa (10%) da Plataforma (R$):</label> <input type="text" id="valor_produto_taxa" name="valor_produto_taxa" value="<?php echo htmlspecialchars($produto['valor_produto_taxa'] ); ?>" somente leitura obrigatório> </div> <!-- Frete Grátis --> <div class="form-group"> <label for="frete_gratis">Frete Grátis:</label> <select id="frete_gratis " name="frete_gratis" onchange="toggleFreteValor()"> <option value="nao" <?php echo ($produto['frete_gratis'] == 'nao') ? 'selected' : ''; ?>> Não</option> <option value="sim" <?php echo ($produto['frete_gratis'] == 'sim') : ''; </div> <!-- Valor do Frete --> <div class="form-group" id="frete-group" style="<?php echo ($produto['valor_frete'] == 'sim' ) ? 'exibir: nenhum;' : 'display:bloco;'; ?>"> <!-- Converte o valor do frete para float e formato --> <?php $valor_frete = str_replace(',', '.', $produto['valor_frete']); $valor_frete = floatval( $valor_frete); ?> <label for="valor_frete">Valor do Frete (R$):</label> <input type="text" id="valor_frete" name="valor_frete" value="<?php echo formato_número($valor_frete, 2, ',', '.'); ?>" oninput="formatarValorFrete(this)"> </div> <!-- Imagens Salvas (até 6) --> <div class="form-group"> <div id="preview"> <?php // Converte uma string de imagens em um array $imagens = isset($produto['imagens']) ? explode(',', $produto['imagens']) : []; $imagens as $index => $imagem): $imagem = trim($imagem));?> <div class="preview-img" id="imagem-<?php echo $index; ?>"> <img src="img_produtos/<?php echo htmlspecialchars($imagem); ?>" alt="Imagem do produto" style="width: 100px; height: 100px;"> <button type="button" class="remove-btn" onclick="removerImagem('<?php echo htmlspecialchars($imagem); ?>', <?php echo $index; ?>)"> <i class="fas fa-trash"></i> <!-- Ícone de lixeira --> </button> </div> <?php endif; endforeach; ?> </div> <label for="produtoImagens">Atualizar Imagens (até 6):</label> <input type="file" id="produtoImagens" name="produtoImagens[]" accept="image/*" multiple> </div> <!-- Botões --> <div class="form-group"> <button type="button" class="btn btn-secondary" onclick="window.history.back();">Voltar</button > <button type="submit" class="btn btn-primary">Salvar Alterações</button> </div> <script src="editar_produto.js"></script> <script> // Função para remover imagem function removemImagem(imagem, index) { if (confirm("Tem certeza que deseja remover esta imagem?")) { const imagensRemovidas = document.getElementById('imagens_removidas').value.split(' ,'); imagensRemovidas.push(imagem); // Adicionada imagem à lista de removidos document.getElementById('imagens_removidas').value = imagensRemovidas.join(','); // Remove uma imagem do preview document.getElementById('imagem-' + index).remove(); } } // Função de pré-visualização de imagens const inputImagens = document.getElementById('produtoImagens'); visualização const = document.getElementById('visualização'); inputImagens.addEventListener('change', function() { // Limpa o preview e continua a exibir imagens anteriores const imagensAtuais = preview.querySelectorAll('.preview-img'); if (imagensAtuais.length > 0) { // Se já houver imagens prévias, mantenha-as no preview imagensAtuais.forEach(img => { preview.appendChild(img); }); } const files = inputImagens.files; // Limita a adição de mais de 6 imagens no total (incluindo as já salvas) if (files.length + imagensAtuais.comprimento > 6) { alert('Você pode carregar até 6 imagens no total (incluindo as já existentes).'); inputImagens.value = ''; // Limpa o campo do arquivo que excede o limite return; } for (deixe i = 0; i < arquivos.length; i++) { const arquivo = arquivos[i]; const leitor = new FileReader(); reader.onload = function(e) { // Cria o contêiner para a nova imagem const div = document.createElement('div'); div.classList.add('preview-img'); div.id = 'nova-imagem-' + i; // Novo ID para a nova imagem // Cria o elemento da imagem const img = document.createElement('img'); img.src = e.target.resultado; img.style.width = '100px'; img.style.height = '100px'; // Cria o botão de remoção com o ícone de lixeira const removeBtn = document.createElement('button'); removeBtn.type = 'botão'; removeBtn.classList.add('remove-btn'); removeBtn.innerHTML = '<i class="fas fa-trash"></i>'; removeBtn.onclick = function() { div.remove(); // Remove uma imagem nova do preview }; // Adiciona a imagem e o botão de remoção ao contêiner div.appendChild(img); div.appendChild(removeBtn); // Adiciona o contêiner ao preview preview.appendChild(div); }; leitor.readAsDataURL(arquivo); // Ler o arquivo de imagem } }); // Função para calcular o valor com a taxa function calcularValorComTaxa() { const valorProduto = parseFloat(document.getElementById('valor_produto').value.replace(',', '.')) || 0; táxons const = 0,10; const valorComTaxa = valorProduto + (valorProduto *taxa); document.getElementById('valor_produto_taxa').value = valorComTaxa.toFixed(2).replace('.', ','); } // Ajusta o valor do frete e exibe ou oculta o campo de frete window.onload = function() { calcularValorComTaxa(); // Calcula o valor com a taxa ao carregar a página }; </script> </form> </body> </html>