...
if (isset($_SESSION['id']) && isset($_GET['id_cliente']) && isset($_GET['id_produto']) && isset($_GET['id_parceiro'])) {
    $id = $_SESSION['id'];
...
<head>
    ...
    <style>
        ...
        .image-slider {
            ...
            margin: 0px auto;
            /* Adiciona espaçamento e centraliza horizontalmente */
            max-width: 500px;
        ...
        .image-slider .main-image img {
            width: 100%;
            /* Responsivo: Ajusta ao tamanho do contêiner */
            max-width: 500px;
        ...
        .image-slider .thumbnail-container {
            ...
            justify-content: center;
            flex-wrap: wrap;
            /* Permite que as miniaturas quebrem linha */
            gap: 10px;
        ...
        a.btn {
            ...
            text-align: center;
            width: auto;
            /* Garante que o tamanho seja consistente */
        }
        ...
        .descricao-box {
            ...
            /* Altura máxima */
            height: auto;
            /* Altura ajustável automaticamente */
            font-size: 0.9em;
            overflow: auto;
            /* Adiciona rolagem caso o conteúdo ultrapasse 150px */
        }
        ...
        @media (max-width: 768px) {
            ...
            .image-slider .main-image img {
                max-width: 90%;
                /* Ajusta a imagem principal */
            }

            .image-slider .thumbnail {
                width: 60px;
                /* Reduz o tamanho das miniaturas em telas menores */
            }
...
<body>
    <div class="container">
        ...
        <?php elseif (!empty($produto)): ?>
            ...
            <?php if (!empty($imagens)): ?>
                <div class="image-slider">
                    <div class="main-image">
                        <img class="active"
                            src="../../paginas/parceiros/produtos/img_produtos/<?= htmlspecialchars($imagens[0]); ?>"
                            alt="Imagem Principal do Produto">
                    </div>
                    <div class="thumbnail-container">
                        <?php foreach ($imagens as $index => $imagem): ?>
                            <img class="thumbnail <?= $index === 0 ? 'active' : ''; ?>"
                                src="../../paginas/parceiros/produtos/img_produtos/<?= htmlspecialchars($imagem); ?>"
                                alt="Imagem do Produto" onclick="changeMainImage(this)">
                        <?php endforeach; ?>
                    </div>
                </div>
            <?php else: ?>
            ...
            ?>
            <p><strong></strong> <?= htmlspecialchars($produto['nome_produto'] ?? 'Produto sem nome'); ?></p>
            <p><strong>Descrição:</strong></p>
            <textarea class="descricao-box"
                readonly><?= nl2br(htmlspecialchars($produto['descricao_produto'] ?? 'Sem descrição disponível')); ?></textarea>
            <p><strong>Preço:</strong> R$ <?= number_format($valor_produto, 2, ',', '.'); ?></p>
            <?php if ($frete_gratis): ?>
                <p><strong style="color: green;">Frete Grátis</strong></p>
            <?php else: ?>
                <p><strong style="color: red;">Frete:</strong> R$
                    <?= number_format($produto['valor_frete'] ?? 0, 2, ',', '.'); ?></p>
            <?php endif; ?>
            ...
            <div class="buttons-container">
                ...
                <form id="formCompra" action="comprar/carrinho.php">
                    ...
                    <?php if (isset($usuario) && $usuario): ?>
                        ...
                        <a href="loja_parceiro.php?id=<?php echo urlencode($id_parceiro); ?>&id_cliente=<?php echo urlencode($id_cliente); ?>"
                            class="btn btn-success">Voltar</a>
                        <button type="button" name="adicionar" class="btn btn-success" onclick="abrirPopup(
                        ...
                        <button type="submit" class="btn btn-danger">Comprar
                        </button>
                    <?php else: ?>
                        <!-- Botões que redirecionam para a página de login -->
                        <a href="loja_parceiro.php" class="btn btn-success">Voltar</a>
                        <a href="#" class="btn btn-success" onclick="abrirPopup(
                            '<?php echo $produto['id_produto']; ?>',
                            '<?php echo $produto['nome_produto']; ?>', 
        ...
        <form id="formCarrinho" action="comprar/carrinho.php">
            <aside id="info">
                <input type="hidden" id="id_cliente" name="id_cliente"
                    value="<?php echo htmlspecialchars($id_cliente); ?>">
